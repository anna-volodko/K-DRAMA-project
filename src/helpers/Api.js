...
export async function searchMedia(searchText, typeMovie, typeShow) {
  try {
    ...
    if (searchText) {
      ...
      if (typeMovie) {
        link = `${BASE_URL}/discover/movie?api_key=${API_KEY}&with_original_language=ko&include_adult=falsesort_by=popularity.desc`;
      } else if (typeShow) {
        link = `${BASE_URL}/discover/tv?api_key=${API_KEY}&with_original_language=ko&include_adult=falsesort_by=popularity.desc`;
      }
    }
